import { renderHook, act } from '@testing-library/react-hooks';
import MockAdapter from 'axios-mock-adapter';

import { AuthProvider, useAuth } from '../../hooks/auth';

import api from '../../services/api';

const apiMock = new MockAdapter(api);

const fakeUserLoginReply = {
  user: {
    id: 'user-23',
    name: 'John Doe',
    email: 'johndoe@example.com',
    avatar:
      'data:image/jpeg;base64,',
  },
  token: 'fdhsafhsdafk',
};

describe('Auth Hook testing', () => {
  it('should be able to sign-in', async () => {
    apiMock.onPost('sessions').reply(200, fakeUserLoginReply);

    const { result, waitForNextUpdate } = renderHook(() => useAuth(), {
      wrapper: AuthProvider,
    });

    const localStorageSetItem = jest.spyOn(Storage.prototype, 'setItem');

    result.current.signIn({
      email: fakeUserLoginReply.user.email,
      password: '123456789',
    });

    await waitForNextUpdate();

    expect(result.current.user.email).toBe(fakeUserLoginReply.user.email);
    expect(localStorageSetItem).toHaveBeenCalledWith(
      '@GoBarber:token',
      fakeUserLoginReply.token,
    );

    expect(localStorageSetItem).toHaveBeenCalledWith(
      '@GoBarber:user',
      JSON.stringify(fakeUserLoginReply.user),
    );
  });

  it('should restore saved data from storage when app inits', () => {
    const localStorageGetItem = jest
      .spyOn(Storage.prototype, 'getItem')
      .mockImplementation(key => {
        switch (key) {
          case '@GoBarber:token':
            return fakeUserLoginReply.token;
          case '@GoBarber:user':
            return JSON.stringify(fakeUserLoginReply.user);
          default:
            return null;
        }
      });

    const { result } = renderHook(() => useAuth(), {
      wrapper: AuthProvider,
    });

    expect(result.current.user.email).toEqual(fakeUserLoginReply.user.email);
    expect(localStorageGetItem).toHaveBeenCalledWith('@GoBarber:token');
    expect(localStorageGetItem).toHaveBeenCalledWith('@GoBarber:user');
  });

  it('should be able to sign-out', () => {
    jest.spyOn(Storage.prototype, 'getItem').mockImplementation(key => {
      switch (key) {
        case '@GoBarber:token':
          return fakeUserLoginReply.token;
        case '@GoBarber:user':
          return JSON.stringify(fakeUserLoginReply.user);
        default:
          return null;
      }
    });

    const { result } = renderHook(() => useAuth(), {
      wrapper: AuthProvider,
    });

    const localStorageRemoveItem = jest.spyOn(Storage.prototype, 'removeItem');

    act(() => {
      result.current.signOut();
    });

    expect(result.current.user).toBeUndefined();
    expect(localStorageRemoveItem).toHaveBeenCalledWith('@GoBarber:token');
    expect(localStorageRemoveItem).toHaveBeenCalledWith('@GoBarber:user');
  });

  it('should be able to update user data', () => {
    const localStorageSetItem = jest.spyOn(Storage.prototype, 'setItem');

    const { result } = renderHook(() => useAuth(), {
      wrapper: AuthProvider,
    });

    const newUserData = {
      id: 'user-23',
      name: 'John New Doe',
      email: 'johnNEWdoe@example.com',
      avatar:
        'data:image/jpeg;base64,',
    };

    act(() => {
      result.current.updateUser(newUserData);
    });

    expect(localStorageSetItem).toHaveBeenCalledWith(
      '@GoBarber:user',
      JSON.stringify(newUserData),
    );

    expect(result.current.user).toEqual(newUserData);
  });
});
